...
class S3Handler:
    ...
    def move_file(self, filename, old_prefix, new_prefix):
        try:
            old_key = f"{old_prefix}/{filename}"
            new_key = f"{new_prefix}/{filename}"
            if self.file_exists(new_key): #prevent overwrite
                new_key = self.number_matching_files(new_key) # append number to filename if it exists
    ...
    async def upload_image(self, group, filename, bytes):
        ...
        proposed_key = f"{group}/thumb/{filename}"
